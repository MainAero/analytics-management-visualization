angular.module('AnalyticsManagementVisualizationApp')
    ...
    .directive('cesiumDisplayHelper', function ($rootScope) {
        return {

            restrict: 'E',
            scope: {
                zones: '=', // two way synchronization
                editable: '@', // string one way
                request: '=',
                result: '@'
            },
            //templateUrl: "components/map/cesiumDrawHelper.html",
            template: "<div id='displayhelper'></div>",

            controller: ['$interval', '$rootScope', 'Cesium', '$scope', '$log', 'zoneService', function ($interval, $rootScope, cesium, $scope, $log, zoneService) {

                $log.debug("drawhelper controller start");

                function initInterval() {
                    $log.info('initInterval');
                    $interval(function () {
                        $log.info('interval - $rootScope.resultCount');
                        $log.info($rootScope.resultCount);

                        if ($scope.curPolygons){
                            $log.debug('polygon set - remove entity');
                            viewer.entities.remove($scope.curPolygons);
                        }

                        if ($scope.request) {
                            $log.debug('request set - add entity');
                            drawPolygonesWithNewHeight($rootScope.resultCount);
                        }
                    }, 5000);
                };
                initInterval();

                function drawPolygonesWithNewHeight(height) {
                    $log.debug("request is set");

                    // TODO: multiple zones?
                    var zoneID = $scope.request.viewZones[0];

                    zoneService.getZone({
                        id: zoneID
                    }, function (data) {
                        $log.debug('all data from the zone: ')
                        $log.info(data);
                        $log.debug('Get zoneID response with data.geometry: : ')
                        $scope.geometryArrays = data.geometry;
                        $log.debug($scope.geometryArrays);
                        $log.debug('and start drawing these zones into the map')

                        $scope.zoneName = data.name;
                        $scope.venueName = data.venue.name;


                        $log.debug('by iterating over these objects:');
                        var positionsArray = [];
                        angular.forEach($scope.geometryArrays, function (geometryObjects, value) {
                            $log.info(geometryObjects);
                            positionsArray.push(geometryObjects[0], geometryObjects[1]);
                        });


                        $log.debug('positionsArray is set: ');
                        $log.info(positionsArray);

                        var curPolygonName = 'Name';
                        var curPolygonDescription = '<h2>' + $scope.zoneName + '</h2><br><p style="text-align: center"> from the venue: ' + $scope.venueName + '</p><br><p style="text-align: center"> With the request result: ' + height + '</p>';
                        var color = Cesium.Color.fromRandom({alpha: 1.0});


                        $log.info('result count rootScope before drawing: ');
                        $log.info($rootScope.resultCount);

                        $scope.curPolygons = viewer.entities.add({
                            name: curPolygonName,
                            description: curPolygonDescription,
                            polygon: {
                                hierarchy: Cesium.Cartesian3.fromDegreesArray(positionsArray),
                                extrudedHeight: height,
                                material: color
                            }
                        });

                        $log.warn('curPolygons ');
                        $log.warn($scope.curPolygons);
                    });
                }

                // create the cesium map
                // var viewer = new cesium.Viewer($scope.mapid, {
                var viewer = new cesium.Viewer("displayhelper", {
                ...
                // DRAW ZONES
                // ----------
                angular.forEach($scope.zones, function (object, value) {
                    var positionsArray = [];
                    var curPolygonName = object.name;
                    var curPolygonDescription = '<p style="text-align: center">' + object.description + '</p>';
                    var color = Cesium.Color.fromRandom({alpha: 1.0});

                    angular.forEach(object.geometry, function (key, value) {
                        positionsArray.push(key[0], key[1]);
                    })

                    var curPolygon = viewer.entities.add({
                        name: curPolygonName,
                        description: curPolygonDescription,
                        polygon: {
                            hierarchy: Cesium.Cartesian3.fromDegreesArray(positionsArray),
                ...
                // get the maps scene to add stuff to it later
                ...
                var DrawHelper = (function () {
                    ...
                    material.uniforms.color = new Cesium.Color(0.0, 0.0, 1.0, 0.2);
                    ...
                    _.DrawHelperWidget = (function () {
                        ...
                        function _(drawHelper, options) {
                            ...
                            var drawOptions = {
                                polygonIcon: "../../assets/img/polygon-icon.png",
                                clearIcon: "../../assets/img/bin-icon.png",
                    ...
                    function createTooltip() {
                        var frameDiv = document.getElementById("draw-info");
                ...
                toolbar.addListener('polygonCreated', function (event) {
                    ...
                    var material = Cesium.Material.fromType(Cesium.Material.ColorType);
                    material.uniforms.color = new Cesium.Color(0.0, 0.0, 1.0, 0.6);

                    console.log(event);
                    createdObjects.push(event);
