angular.module('AnalyticsManagementVisualizationApp')
    ...
    .directive('cesiumDrawHelper', function ($rootScope) {
        return {
            ...
            controller: ['Cesium', '$scope', function (cesium, $scope) {

                console.log("drawhelper controller start");
                console.log($scope);
                ...
                if ($rootScope.listOfImportedZones) {
                    console.log($rootScope.listOfImportedZones);

                    angular.forEach($rootScope.listOfImportedZones, function (geometryArray, value) {
                        var positionsArray = [];
                        var materialForCurPolygon = Cesium.Material.fromType(Cesium.Material.ColorType);

                        angular.forEach(geometryArray.geometry, function (key, value) {
                            positionsArray.push(key[0], key[1]);
                        })
                        var curPolygon = new DrawHelper.PolygonPrimitive({
                            positions: Cesium.Cartesian3.fromDegreesArray(positionsArray),
                            material: materialForCurPolygon,
                            outline: true
                        });
                        scene.primitives.add(curPolygon);
                    });
                }
