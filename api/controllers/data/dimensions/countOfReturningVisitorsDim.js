...
exports.getName = function () {
    return constants.RETURNING_VISITORS;
};
...
exports.getData = function (dateResolution, startD, endD, view_id, callback) {
    if (dateResolution == constants.NO_DATE)
        ...
        dimUtils.getDataWithDateCallback(dateResolution, startD, endD, view_id, constants.RETURNING_VISITORS, constants.COUNT_MET, dimUtils.valueCountEvaluator, callback)
};
...
exports.getDataNoDate = function (dateResolution, startD, endD, view_id, callback) {
    ...
    var groupByTarget = {
        $group:{
            ...
            visits:{$push:{date_entered:"$value.visitIntervals.date_entered"}}
        }
    }
    ...
    var returningCondition = {
        $match: {
            "newVisit": {$lte:startD},
            "visits":{$elemMatch:{"date_entered":{$gte:startD,$lte:endD}}}
        }
    }
    ...
    viewVisits.aggregate(
        [
            ...
            groupByTarget,
            //Filter documents that have a visit before start date and date have at least one visit between start and end date
            returningCondition,
